...
class _TERRAINDataset(Dataset):
    def __init__(self, root, num_points, split='train', with_normalized_coords=True, holdout_area=5):
        ...
        num_areas = len(os.listdir(self.root))-1
        areas = []
        if self.split == 'train':
            for a in range(1, (num_areas+1)): # Fixat så alla areas tas med
...
class TERRAIN(dict):
    def __init__(self, root, num_points, split=None, with_normalized_coords=True, holdout_area=5):
        ...
        if split is None:
            ...
            split = [split]
        for s in split:
            self[s] = _TERRAINDataset(root=root, num_points=num_points, split=s,
